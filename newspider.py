...
def scrap_page_data(url):
    r = requests.get("https://www.booking.com/" + url)
...
def get_page(url,i,link_list=[]):
    try:
        ...
        url_list1=soup.find_all("a", {"class": "sr_item_photo_link sr_hotel_preview_track"})
        with Pool(cpu_count()) as pool:
            link_list.append(pool.map(scrap_page_data,url_list1))
        if i<40:
            get_page(
